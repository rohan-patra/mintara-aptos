...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Using the direct function call payload with explicit resource address
      const payload = {
      ...
      if (errorMessage.includes("INVALID_ARGUMENT_ERROR")) {
        return NextResponse.json({
          success: false,
          error: "Invalid arguments to sell_tokens function. Check the contract signature.",
          contractAddress,
          resourceAddress,
          details: errorMessage,
          correctSignature: "sell_tokens(seller: &signer, amount: u64)",
          note: "The 'seller' parameter is implicit (transaction signer)"
        }, { status: 400 });
      }
      ...
      if (errorMessage.includes("INSUFFICIENT_TOKEN_BALANCE")) {
        return NextResponse.json({
          success: false,
          error: "Insufficient token balance to sell the requested amount.",
          amount,
          contractAddress
        }, { status: 400 });
      }
      ...
      return NextResponse.json({
        success: false, 
        error: errorMessage,
        message: "Error while trying to sell tokens",
