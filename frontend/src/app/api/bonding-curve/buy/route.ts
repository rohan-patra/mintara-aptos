import { AptosClient, AptosAccount, HexString, Types, BCS } from 'aptos';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { amount, moduleAddress, resourceAddress: requestedResourceAddress } = body;
    ...
    console.log("Replacing original @mintara address with contract address in the module");
    ...
    try {
      ...
      console.log("APT balance:", aptBalance, `(${aptBalance / 100000000} APT)`);
    } catch (error) {
    ...
    try {
      ...
      return NextResponse.json({
        success: false,
        error: "Bonding curve module not found at the specified address. Please deploy the contract first and initialize it.",
        deploymentInstructions: {
          ...
          step3: "After deployment, initialize the bonding curve using the initialize endpoint",
    ...
    if (!resourceAddress) {
      return NextResponse.json({
        success: false,
        error: "Could not determine resource address. Please provide it explicitly.",
        contractAddress
      }, { status: 400 });
    }
    ...
    try {
      ...
      const capsResource = resources.find(r => 
    ...
    console.log("Trying direct function call with resource address:", JSON.stringify(payload));
    ...
    while (retries < maxRetries) {
      try {
        ...
        txnInfo = await client.getTransactionByHash(txnResult.hash);
        console.log("Transaction status:", txnInfo?.success ? "Success" : "Failed");
        break;
      } catch (error) {
        console.log(`Error checking transaction (attempt ${retries + 1}/${maxRetries}):`, error);
        retries++;
    ...
    try {
      ...
      if (errorMessage.includes("bonding_curve::initialize")) {
        return NextResponse.json({
          success: false,
          error: "The bonding curve has not been initialized yet. Please initialize it first.",
          moduleExists: true,
          contractAddress
        }, { status: 400 });
      }
      ...
      if (errorMessage.includes("INVALID_ARGUMENT_ERROR")) {
        return NextResponse.json({
          success: false,
          error: "Invalid arguments to buy_tokens function. Check the contract signature.",
          contractAddress,
          resourceAddress,
      ...
      if (errorMessage.includes("E_NOT_INITIALIZED")) {
        return NextResponse.json({
          success: false,
          error: "Contract resources not initialized properly. Please provide the correct resource address or reinitialize the bonding curve.",
          contractAddress,
          resourceAddress,
          details: errorMessage
        }, { status: 400 });
      ...
      if (errorMessage.includes("INSUFFICIENT_APT_BALANCE")) {
        return NextResponse.json({
          success: false,
          error: "Insufficient APT balance to buy the requested tokens.",
          amount,
          contractAddress
        }, { status: 400 });
      }
      ...
      return NextResponse.json({
        success: false,
        error: errorMessage,
        message: "Error while trying to buy tokens",
